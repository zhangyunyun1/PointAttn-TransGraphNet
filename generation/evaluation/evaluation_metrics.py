...
from loss_functions import emdModule  # 导入 EMD 模块

chamfer = chamfer_3DDist()

# 实例化 EMD 计算模块
emd_calculator = emdModule().cuda()

def distChamferCUDA(x, y):
    return chamfer(x, y)[:2]

def emd_approx(sample, ref, eps=0.05, iters=3000):
    """
    Computes the Earth Mover's Distance (EMD) between two point clouds using the emdModule.
    
    Args:
        sample (torch.Tensor): Generated point clouds, shape (B, N, 3)
        ref (torch.Tensor): Reference point clouds, shape (B, N, 3)
    ...
    """
    # 确保 EMD 模块在与输入相同的设备上
    emd_calculator.to(sample.device)
    
    # 使用 emdModule 计算 EMD
    dist, assignment = emd_calculator(sample, ref, eps, iters)
    
    # dist 是 [B, N]，取平方根得到 L2 距离，然后对每个点云取均值
...
def distChamfer(a, b):
    ...
    diag_ind = torch.arange(0, num_points).to(a.device).long()
...
def knn(Mxx, Mxy, Myy, k, sqrt=False):
    n0 = Mxx.size(0)
    n1 = Myy.size(0)
    label = torch.cat((torch.ones(n0), torch.zeros(n1))).to(Mxx.device)
    M = torch.cat([
        torch.cat((Mxx, Mxy), 1),
    ...
    if sqrt:
        M = M.abs().sqrt()
    INFINITY = float('inf')
    val, idx = (M + torch.diag(INFINITY * torch.ones(n0 + n1).to(Mxx.device))).topk(
        k, 0, False)

    count = torch.zeros(n0 + n1).to(Mxx.device)
    for i in range(0, k):
        count = count + label.index_select(0, idx[i])
    pred = torch.ge(count, (float(k) / 2) * torch.ones(n0 + n1).to(Mxx.device)).float()
...
def lgan_mmd_cov(all_dist):
    ...
    cov = torch.tensor(cov).to(all_dist.device)
    return {
        'lgan_mmd': mmd,
        'lgan_cov': cov,
...
def jensen_shannon_divergence(P, Q):
    ...
    if not np.allclose(res, res2, atol=1e-5, rtol=0):
