...
def normalize_point_clouds(pcs, mode, logger):
    ...
    for i in tqdm(range(pcs.size(0)), desc='Normalize'):
        pc = pcs[i]
        if mode == 'shape_unit':
            shift = pc.mean(dim=0, keepdim=True)  # (1, 3)
...
parser.add_argument('--ckpt', type=str,
                    default='logs_gen/snowflake_cd_only/airplane/GEN_2025_04_10__15_51_11/ckpt_0.000000_60000.pt')
...
for i in tqdm(range(num_batches), desc='Generate'):
    with torch.no_grad():
        batch_size = args.batch_size if (i+1)*args.batch_size <= num_samples else num_samples - i*args.batch_size
        z = torch.randn([batch_size, ckpt['args'].latent_dim]).to(args.device)
        x = model.sample(z)
